...
* **Official ISA Compliance Vectors:** The `Top_Module` was rigorously tested by loading and executing the specialized `.hex` firmware streams directly into the `IMEM`. Every individual instruction category (such as arithmetic, logical shifts, load/store boundaries, and conditional branches) was stressed using these official binary patterns to evaluate edge cases and overflow conditions.
* **Function-by-Function Validation:** By running these official test blocks the core has proven to comply with the official RISC-V standard at a bit-accurate level.
